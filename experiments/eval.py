...
def do_eval(query_data, database_data_all, image_model, text_model, predict_model, bit, cuda, batch_size, epoch,
    ...
    query_dataloader = DataLoader(query_data, batch_size=batch_size, drop_last=True, shuffle=True,
                                  num_workers=num_works, pin_memory=False)
    database_dataloader = DataLoader(database_data_all, batch_size=batch_size, drop_last=True, shuffle=True,
                                     num_workers=num_works, pin_memory=False)
    ...
    plot_function(query_image_buffer, query_text_buffer, database_image_buffer, database_text_buffer,
                  query_label_buffer, database_label_buffer, save_dir)

    if image2text_q >= max_map["old"]["max_mapi2t_q"]:
        max_map["old"]["max_mapi2t_q"] = image2text_q
        best_epoch["old"]["best_epoch_q"] = epoch
    if text2image_q >= max_map["old"]["max_mapt2i_q"]:
        max_map["old"]["max_mapt2i_q"] = text2image_q
        best_epoch["old"]["best_epoch_q"] = epoch

    if image2text_d >= max_map["old"]["max_mapi2t_d"]:
        max_map["old"]["max_mapi2t_d"] = image2text_d
        best_epoch["old"]["best_epoch_d"] = epoch
    if text2image_d >= max_map["old"]["max_mapt2i_d"]:
    ...
    logger.info("image2text_map_d:{}     text2image_map_d:{}     ValidateTime:{}".format(image2text_d,
                                                                                         text2image_d,
                                                                                         time_elapsed_1))
    logger.info("image2text_map_q:{}     text2image_map_q:{}     ValidateTime:{}".format(image2text_q,
                                                                                         text2image_q,
                                                                                         time_elapsed_1))
...
def do_eval_extend(query_data_old, query_data, database_data_old, database_data_new, database_data_all, image_model,
    ...
    query_label_buffer, database_label_buffer = torch.empty(len(query_data), all_num, dtype=torch.float), torch.empty(
        len(database_data_all), all_num, dtype=torch.float)
    old_query_label_buffer, old_database_label_buffer, new_database_label_buffer = \
        torch.empty(len(query_data_old), all_num, dtype=torch.float), \
        torch.empty(len(database_data_old), all_num, dtype=torch.float), \
        torch.empty(len(database_data_new), all_num, dtype=torch.float)
...
def retrivel(query_data, image_model, text_model, batch_size, cuda, bit, map, best_epoch, epoch,
             num_works, all_num, flag):
    query_dataloader = DataLoader(query_data, batch_size, shuffle=True, pin_memory=True, drop_last=True,
                                  num_workers=num_works)
    # retrival_dataloader = DataLoader(retrival_data, batch_size, shuffle=True, pin_memory=True, drop_last=True,
    #                                  num_workers=num_works)
    ...
    # image_database, text_database, label_database = torch.zeros(len(retrival_data), bit), \
    #                                                 torch.zeros(len(retrival_data), bit), \
    #                                                 torch.zeros(len(retrival_data), all_num)
    if cuda:
        image_hash_buffer = image_hash_buffer.cuda()
        text_hash_buffer = text_hash_buffer.cuda()
        label_buffer = label_buffer.cuda()

        image_database = image_hash_buffer.cuda()
        text_database = text_hash_buffer.cuda()
        label_database = label_buffer.cuda()
    image_model.eval()
    text_model.eval()
    with torch.no_grad():
        ...
        # for data1 in retrival_dataloader:
        #     image1 = data1["image"]
        #     text1 = data1["text"]
        #     label1 = data1["label"]
        #     index1 = data1["index"].numpy()
        #     if cuda:
        #         image1 = image1.cuda()
        #         text1 = text1.cuda()
        #         label1 = label1.cuda()
        #     image_results = image_model(image1)
        #     text_results = text_model(text1)
        #     image_hash = torch.sign(image_results)
        #     text_hash = torch.sign(text_results)
        #     image_database[index, :] = image_hash
        #     text_database[index, :] = text_hash
        #     label_database[index, :] = label1
